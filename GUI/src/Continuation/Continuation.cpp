...
void cContinuation::UWC_p2p(string inputfilename,string outputfilename,double height1,double height2,int extNum,int num_thread,bool show)
{
    ...
    {
        Msg::Error("The output file format is not supported: %s",ext_outputfile.c_str());
    }
    //delete data array
    delete[] indata;
    delete[] outdata;
...
int cContinuation::Getkernel_p2p_new(GrdHead grdhead, double rph, double** kernel, int num_thread)
{
    ...
    for(int irow=0; irow<grdhead.rows; irow++)
    {
        ...
        // multibar.Update();
        ind_progress++;
        Msg::StatusBar(true,"Kernal Matrix Calculation Progress: %d%%",int((ind_progress/grdhead.rows)*100));
    }
...
int cContinuation::Getkernel_p2s_new(GrdHead grdhead, double h1,double* topo2, double** kernel, int num_thread)
{
    Msg::Info("Calculating new kernel of upward continuation from plane to surface");
    double dx=(grdhead.bounds[1]-grdhead.bounds[0])/(grdhead.cols-1);
    double dy=(grdhead.bounds[3]-grdhead.bounds[2])/(grdhead.rows-1);
    int datanum=grdhead.rows*grdhead.cols;
    //omp parallel
...
void cContinuation::UWC_p2s(string inputfilename,string outputfilename,double height1,string topoFile, int extNum, int num_thread)
{
    ...
    {
        Msg::Error("Input topography grid file does not match the input field grid data, please check");
        return ;
    }
    //3. kernel mat.
    ...
    Msg::Info("calculating kernal matrix");
    ...
    Msg::Info("Calculating is finished");
    ...
    {
        Msg::Error("The output file format is not supported: ",ext_outputfile.c_str());
        return;
    }
    //delete data array
    delete[] indata;
...
void cContinuation::DWC_p2p(string inputfilename,string outputfilename,double height1,double height2,int extNum,double DWC_parameter,int DWC_method,int num_thread)
{
    //1. read grd data
    GrdHead grdhead;
    double* indata = NULL;
    ...
    double dy=(grdhead.bounds[3]-grdhead.bounds[2])/(grdhead.rows-1);
    Msg::Info("*********************************************************");
    Msg::Info("Dimension of data: %d x %d",grdhead.rows,grdhead.cols);
    Msg::Info("Size of kernel mat.: %d x %d",grdhead.rows*grdhead.cols,grdhead.rows*grdhead.cols);
    int size_kernel=int(sizeof(double)*pow(grdhead.rows*grdhead.cols,2)/pow(1024.0,2));
    Msg::Info("Memory of kernel mat.: %d Mb",size_kernel);
    Msg::Info("Downward continuation: %f point spacing",rph/dx);
    Msg::Info("*********************************************************");
    ...
    Msg::Info("Calculating kernel matrix");
    Getkernel_p2p_new(grdhead,rph,G,num_thread);
    Msg::Info("The kernel matrix has been calculated");
    ...
    {
        case DWC_LANDWEBER:
            DWC_s2p_LandweberIter(G,dataout,indata,grdhead,extNum,num_thread,outputfilename,DWC_parameter);
            break;
        default:
            Msg::Error("The downward continuation method is not available yet");
            exit(0);
    }
    //5. write result
...
void cContinuation::DWC_s2p_LandweberIter(double** G,double* x, double* b,
    GrdHead grdhead,int extNum,int num_thread,string outputfile,double iter_number)
{
    int kmax=100;
    if(iter_number>0)kmax=int(iter_number);
    double REmin=1E-8;
    Msg::Info("Landweber iteration method: kmax=%d, REmin=%.2E",kmax,REmin);
    ...
    if(system(command.c_str()))
    {
        ...
        Msg::Info("Create directory to save temporary resut: %s",path_tempResult.c_str());
    }
    ...
    if(!fout_log)
    {
        Msg::Error("Open file failed: %s",filename_log.c_str());
        return ;
    }
    //iteration
...
double cContinuation::Norm2(double* x,const int num)
{
    double sum=0;
    for(int i=0;i<num;i++)
    {
...
double cContinuation::Norm2_Gradient(double* result,GrdHead grdhead)
{
...
void cContinuation::DWC_s2p(string inputfilename,string outputfilename,string topoFile,
    double height2,int extNum,double DWC_parameter,int DWC_method, int num_thread)
{
    const double daierta = 0.00001;										//Iterative termination condition
    Msg::Info("The maximum iteration number: %d, Iterative termination condition: %.2E",int(DWC_parameter),daierta);
    //1. read grd data
    GrdHead grdhead;
    double* indata = NULL;
    if (!(indata = ReadGrd(inputfilename, grdhead,extNum)))return ;
    ...
    int size_kernel=int(sizeof(double)*pow(grdhead.rows*grdhead.cols,2)/pow(1024.0,2));
    Msg::Info("Memory of kernel mat.: %d Mb",size_kernel);
    Msg::Info("Point spacing dx=%f, dy=%f",dx,dy);
    Msg::Info("Downward continuation: %f point spacing to %f",((grdhead_topo.bounds[5]-height2)/dx),((grdhead_topo.bounds[4]-height2)/dx));
    Msg::Info("*********************************************************");
    ...
    switch(DWC_method)
    {
        case DWC_LANDWEBER:
            ...
            break;
        default:
            Msg::Error("The method of downward continuation is not available yet.");
            return ;
    }
    ...
    }else if(ext_outputfile=="vtk" || ext_outputfile=="VTK")
    {
        ...
        string filename_outfile=getFileName(outputfilename);
...
int cContinuation::SaveGrd2VTK_topo(string outputfile,GrdHead grdhead,double* data,double* topo)
{
    //vtk format
    ofstream fout(outputfile);
    if(!fout)
    {
        Msg::Error("Open file failed: %s.vtk",outputfile.c_str());
        return 1;
    }
    double dx=(grdhead.bounds[1]-grdhead.bounds[0])/(grdhead.cols-1);
    double dy=(grdhead.bounds[3]-grdhead.bounds[2])/(grdhead.rows-1);
